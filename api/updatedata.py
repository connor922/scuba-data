...
class JobTitleMatch:
    ...
    def __init__(self, jobtitles = [], kw = [], exclude_kw = [], sen = [], exclude_sen = [], jt = [], exclude_jt = []):
        ...
        self.jobtitles_clean = [unidecode(re.sub(r'[\W_]+ |(/)', " ", j)).lower().strip() for j in jobtitles]
        self.kw_clean = [unidecode(re.sub(r'[\W_]+ |(/)', " ", j)).lower().strip() for j in kw]
        self.exclude_kw_clean = [unidecode(re.sub(r'[\W_]+ |(/)', " ", j)).lower().strip() for j in exclude_kw]
        self.sen_clean = [unidecode(re.sub(r'[\W_]+ |(/)', " ", j)).lower().strip() for j in sen]
        self.exclude_sen_clean = [unidecode(re.sub(r'[\W_]+ |(/)', " ", j)).lower().strip() for j in exclude_sen]
        self.jt_clean = [unidecode(re.sub(r'[\W_]+ |(/)', " ", j)).lower().strip() for j in jt]
...
class CompanyMatch(JobTitleMatch):
    ...
    def remove_company_extensions(self):
        ...
        def clean(comps, company_adds):
            ...
            for i in comps:
                comp = i.split(' ')
                comp = [i.replace('/','') for i in comp if i.replace('/','') not in company_adds]
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self):
        ...
        self.wfile.write(json.dumps({"jt_report": a[0], "jt_report_sum": a[1], "jt_ucounts": unique_jts, "comp_report": b[0], "comp_report_sum": b[1], "comp_ucounts": unique_comps}).encode())
        return
